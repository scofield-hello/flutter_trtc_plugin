 ...
 * - 媒体音量：手机专门为播放器音乐和电影而设计的音量类型，不会做 AEC，通过调节手机的音量调节键可以将音量调成0。
 ...
 *
 * @note 由于安装包体积增量的考虑，只有企业版（Enterprise）和专业版（Professional）的 SDK 中才包含声学算法库。
...
typedef NS_ENUM(NSInteger, TRTCLogLevel) {
    ...
    TRTCLogLevelInfo = 2,      ///< 输出 INFO，WARNNING，ERROR 和 FATAL 级别的 Log
    TRTCLogLevelWarn = 3,      ///< 只输出WARNNING，ERROR 和 FATAL 级别的 Log
