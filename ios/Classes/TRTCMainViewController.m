...
- (void)initUI {
    ...
    _backBtn.frame = CGRectMake(10, 30, 40, 40);
    [_backBtn setImage:[UIImage imageNamed:@"back_full"] forState:UIControlStateNormal];
    [_backBtn addTarget:self action:@selector(backInfo) forControlEvents:UIControlEventTouchUpInside];
...
//    self.title = _roomID;
    ...
    _muteSwitch = NO;
    _btnMute = [self createBottomBtnIcon:@"muteAudio" action:@selector(clickMute:)];
    
    _showLogType = 0;
    _isCamera = NO;
    _btnLog = [self createBottomBtnIcon:@"camera2" action:@selector(clickLog:)];
...
- (void)clickLog:(UIButton *)btn {
    _isCamera = !_isCamera;
           [_trtc switchCamera];
           [[self class] setSettingValue:@(!_isCamera) key:SETTING_CAMERA];
          [btn setImage:[UIImage imageNamed:(_isCamera ? @"camera" : @"camera2")] forState:UIControlStateNormal];
}
...
- (void)clickMute:(UIButton *)btn {
    ...
    [_btnMute setImage:[UIImage imageNamed:(_muteSwitch ? @"unmuteAudio" : @"muteAudio")] forState:UIControlStateNormal];
}
